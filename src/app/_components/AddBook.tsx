...
import {isBookExists} from "@/lib/libraryValidation";
...
const bookSchema = z.object({
    ...
    published_year: z.string().regex(/[TZ.\-:]/,{message : "date cannot be empty"}).transform((val) => new Date(val).toISOString()).refine((p_year) => p_year <= new Date().toISOString(),{message : "published date cannot be greater than today's date"}), // Date for the published year
...
export default function AddBook({selectedName,className,mutate}){
    ...
    const form = useForm<z.infer<typeof bookSchema>>({
        ...
        defaultValues : {
            bookId: "", // Default 4-digit book ID
            name: "", // Default book name
            description: "", // Default description
            price: '', // Default price as a string to be transformed later
            publisher: "", // Default publisher
            published_year: "", // Default to today's date in YYYY-MM-DD format
            bookImg: "", // Default valid image URL
            total_quantity: '', // Default total quantity as a string to be transformed
      ...
      function onSubmit(values: z.infer<typeof bookSchema>) {
        ...
        isBookExists(values.bookId)
        .then((data) => {
          if(data){
            closeDialog();
            toast.error('book id is already exists', {
              position: "top-right",
              autoClose: 2000,
    ...
    return (
        <Dialog open={open} onOpenChange={handleDialogChange}>
                  <DialogTrigger asChild>
                    <Button className={className}>Add book</Button>
                  </DialogTrigger>
                  <DialogContent>
                    <DialogHeader>
                              ...
                              <FormField
                                ...
                                render={({ field }) => (
                                  <FormItem>
                                    ...
                                    <FormControl>
                                      <Input {...field} placeholder="A123F" className="border border-gray-500" type="text"  />
                                    </FormControl>
                                    <FormMessage/>
                                  </FormItem>
                                )}
